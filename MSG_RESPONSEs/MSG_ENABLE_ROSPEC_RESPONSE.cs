...
// Type: Org.LLRP.LTK.LLRPV1.MSG_ENABLE_ROSPEC_RESPONSE
...
namespace Org.LLRP.LTK.LLRPV1
{
  public class MSG_ENABLE_ROSPEC_RESPONSE : Message
  {
    ...
    public static MSG_ENABLE_ROSPEC_RESPONSE FromString(string str)
    {
      ...
      try
      {
        enableRospecResponse.MSG_ID = Convert.ToUInt32(XmlUtil.GetNodeAttrValue(documentElement, "MessageID"));
      }
      catch
      {
      }
      try
      {
        ...
        {
          if (xmlNodes.Count != 0)
            enableRospecResponse.LLRPStatus = PARAM_LLRPStatus.FromXmlNode(xmlNodes[0]);
        }
      }
      catch
      {
