...
// Type: Org.LLRP.LTK.LLRPV1.MSG_STOP_ROSPEC_RESPONSE
...
namespace Org.LLRP.LTK.LLRPV1
{
  public class MSG_STOP_ROSPEC_RESPONSE : Message
  {
    ...
    public static MSG_STOP_ROSPEC_RESPONSE FromString(string str)
    {
      ...
      MSG_STOP_ROSPEC_RESPONSE stopRospecResponse = new MSG_STOP_ROSPEC_RESPONSE();
      try
      {
        stopRospecResponse.MSG_ID = Convert.ToUInt32(XmlUtil.GetNodeAttrValue(documentElement, "MessageID"));
      }
      catch
      {
      }
      try
      {
        ...
        {
          if (xmlNodes.Count != 0)
            stopRospecResponse.LLRPStatus = PARAM_LLRPStatus.FromXmlNode(xmlNodes[0]);
        }
      }
      catch
      {
    ...
    public static MSG_STOP_ROSPEC_RESPONSE FromBitArray(
      ref BitArray bit_array,
      ref int cursor,
      int length)
    {
      if (cursor > length)
        return (MSG_STOP_ROSPEC_RESPONSE) null;
      ArrayList arrayList = new ArrayList();
      MSG_STOP_ROSPEC_RESPONSE stopRospecResponse = new MSG_STOP_ROSPEC_RESPONSE();
      cursor += 6;
      if ((int) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 10) != (int) stopRospecResponse.msgType)
      {
        cursor -= 16;
        return (MSG_STOP_ROSPEC_RESPONSE) null;
      }
      stopRospecResponse.msgLen = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      stopRospecResponse.msgID = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      stopRospecResponse.LLRPStatus = PARAM_LLRPStatus.FromBitArray(ref bit_array, ref cursor, length);
      return stopRospecResponse;
    }
  }
}
