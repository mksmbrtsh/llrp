...
// Type: Org.LLRP.LTK.LLRPV1.MSG_START_ROSPEC_RESPONSE
...
namespace Org.LLRP.LTK.LLRPV1
{
  public class MSG_START_ROSPEC_RESPONSE : Message
  {
    ...
    public static MSG_START_ROSPEC_RESPONSE FromString(string str)
    {
      ...
      MSG_START_ROSPEC_RESPONSE startRospecResponse = new MSG_START_ROSPEC_RESPONSE();
      try
      {
        startRospecResponse.MSG_ID = Convert.ToUInt32(XmlUtil.GetNodeAttrValue(documentElement, "MessageID"));
      }
      catch
      {
      }
      try
      {
        ...
        {
          if (xmlNodes.Count != 0)
            startRospecResponse.LLRPStatus = PARAM_LLRPStatus.FromXmlNode(xmlNodes[0]);
        }
      }
      catch
      {
    ...
    public static MSG_START_ROSPEC_RESPONSE FromBitArray(
      ref BitArray bit_array,
      ref int cursor,
      int length)
    {
      if (cursor > length)
        return (MSG_START_ROSPEC_RESPONSE) null;
      ArrayList arrayList = new ArrayList();
      MSG_START_ROSPEC_RESPONSE startRospecResponse = new MSG_START_ROSPEC_RESPONSE();
      cursor += 6;
      if ((int) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 10) != (int) startRospecResponse.msgType)
      {
        cursor -= 16;
        return (MSG_START_ROSPEC_RESPONSE) null;
      }
      startRospecResponse.msgLen = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      startRospecResponse.msgID = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      startRospecResponse.LLRPStatus = PARAM_LLRPStatus.FromBitArray(ref bit_array, ref cursor, length);
      return startRospecResponse;
    }
  }
}
