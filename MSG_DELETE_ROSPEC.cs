...
// Type: Org.LLRP.LTK.LLRPV1.MSG_DELETE_ROSPEC
...
namespace Org.LLRP.LTK.LLRPV1
{
  public class MSG_DELETE_ROSPEC : Message
  {
    ...
    public override string ToString()
    {
      ...
      return str + "</DELETE_ROSPEC>";
    }

    public static MSG_DELETE_ROSPEC FromString(string str)
    {
      ...
      MSG_DELETE_ROSPEC msgDeleteRospec = new MSG_DELETE_ROSPEC();
      try
      {
        msgDeleteRospec.MSG_ID = Convert.ToUInt32(XmlUtil.GetNodeAttrValue(documentElement, "MessageID"));
      }
      catch
      {
      }
      string nodeValue = XmlUtil.GetNodeValue(documentElement, "ROSpecID");
      msgDeleteRospec.ROSpecID = (uint) Util.ParseValueTypeFromString(nodeValue, "u32", "");
    ...
    public static MSG_DELETE_ROSPEC FromBitArray(
      ref BitArray bit_array,
      ref int cursor,
      int length)
    {
      if (cursor > length)
        return (MSG_DELETE_ROSPEC) null;
      ArrayList arrayList = new ArrayList();
      MSG_DELETE_ROSPEC msgDeleteRospec = new MSG_DELETE_ROSPEC();
      cursor += 6;
      if ((int) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 10) != (int) msgDeleteRospec.msgType)
      {
        cursor -= 16;
        return (MSG_DELETE_ROSPEC) null;
      }
      msgDeleteRospec.msgLen = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      msgDeleteRospec.msgID = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      if (cursor > length)
        throw new Exception("Input data is not a complete LLRP message");
      int field_len = 32;
      object obj;
      Util.ConvertBitArrayToObj(ref bit_array, ref cursor, out obj, typeof (uint), field_len);
      msgDeleteRospec.ROSpecID = (uint) obj;
      return msgDeleteRospec;
    }
  }
}
