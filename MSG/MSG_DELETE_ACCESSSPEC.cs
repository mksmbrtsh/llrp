...
// Type: Org.LLRP.LTK.LLRPV1.MSG_DELETE_ACCESSSPEC
...
namespace Org.LLRP.LTK.LLRPV1
{
  public class MSG_DELETE_ACCESSSPEC : Message
  {
    ...
    public override string ToString()
    {
      ...
      return str + "</DELETE_ACCESSSPEC>";
    }

    public static MSG_DELETE_ACCESSSPEC FromString(string str)
    {
      ...
      MSG_DELETE_ACCESSSPEC deleteAccessspec = new MSG_DELETE_ACCESSSPEC();
      try
      {
        deleteAccessspec.MSG_ID = Convert.ToUInt32(XmlUtil.GetNodeAttrValue(documentElement, "MessageID"));
      }
      catch
      {
      }
      string nodeValue = XmlUtil.GetNodeValue(documentElement, "AccessSpecID");
      deleteAccessspec.AccessSpecID = (uint) Util.ParseValueTypeFromString(nodeValue, "u32", "");
      return deleteAccessspec;
    }
    ...
    public static MSG_DELETE_ACCESSSPEC FromBitArray(
      ref BitArray bit_array,
      ref int cursor,
      int length)
    {
      if (cursor > length)
        return (MSG_DELETE_ACCESSSPEC) null;
      ArrayList arrayList = new ArrayList();
      MSG_DELETE_ACCESSSPEC deleteAccessspec = new MSG_DELETE_ACCESSSPEC();
      cursor += 6;
      if ((int) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 10) != (int) deleteAccessspec.msgType)
      {
        cursor -= 16;
        return (MSG_DELETE_ACCESSSPEC) null;
      }
      deleteAccessspec.msgLen = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      deleteAccessspec.msgID = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      if (cursor > length)
        throw new Exception("Input data is not a complete LLRP message");
      int field_len = 32;
      object obj;
      Util.ConvertBitArrayToObj(ref bit_array, ref cursor, out obj, typeof (uint), field_len);
      deleteAccessspec.AccessSpecID = (uint) obj;
      return deleteAccessspec;
    }
  }
}
