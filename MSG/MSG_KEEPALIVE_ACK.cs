...
// Type: Org.LLRP.LTK.LLRPV1.MSG_KEEPALIVE_ACK
...
namespace Org.LLRP.LTK.LLRPV1
{
  public class MSG_KEEPALIVE_ACK : Message
  {
    ...
    public static MSG_KEEPALIVE_ACK FromString(string str)
    {
      ...
      MSG_KEEPALIVE_ACK msgKeepaliveAck = new MSG_KEEPALIVE_ACK();
      try
      {
        msgKeepaliveAck.MSG_ID = Convert.ToUInt32(XmlUtil.GetNodeAttrValue(documentElement, "MessageID"));
      }
      catch
      {
      }
    ...
    public static MSG_KEEPALIVE_ACK FromBitArray(
      ref BitArray bit_array,
      ref int cursor,
      int length)
    {
      if (cursor > length)
        return (MSG_KEEPALIVE_ACK) null;
      ArrayList arrayList = new ArrayList();
      MSG_KEEPALIVE_ACK msgKeepaliveAck = new MSG_KEEPALIVE_ACK();
      cursor += 6;
      if ((int) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 10) != (int) msgKeepaliveAck.msgType)
      {
        cursor -= 16;
        return (MSG_KEEPALIVE_ACK) null;
      }
      msgKeepaliveAck.msgLen = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      msgKeepaliveAck.msgID = (uint) (ulong) Util.CalculateVal(ref bit_array, ref cursor, 32);
      return msgKeepaliveAck;
    }
  }
}
